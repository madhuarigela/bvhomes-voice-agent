...
def build_llm():
    ...
    # Gemini through LiveKit Inference.  This uses the LIVEKIT_* credentials
    # already required by the worker and does not require a separate Google key.
    # Keep the provider prefix: it is how LiveKit routes the request.
    return inference.LLM("google/gemini-2.5-flash")


class BVHomesAgent(Agent):
    def __init__(self, tools: BVHomesTools) -> None:
        super().__init__(
...
async def entrypoint(ctx: JobContext) -> None:
    ...
    async def persist_completed_conversation():
        """Persist call data without ever making session shutdown fail."""
        try:
            logger.info(
                "Usage for room=%s: %s",
                ctx.room.name,
        ...
        try:
            ...
            await lead_store.save_conversation(
                ConversationRecord(
                    ...
                    duration_seconds=max(0, int((ended_at - session_started_at).total_seconds())),
                    ...
                    requirements=summary["requirements"] or str(
                        lead.get("furniture_requirement", "")
                    ),
            ...
            # Session data is valuable, but a DB/LLM outage must never affect a call.
            logger.exception("Failed to persist completed conversation for room=%s", ctx.room.name)

    ctx.add_shutdown_callback(persist_completed_conversation)

    try:
        await session.start(
            agent=BVHomesAgent(tools=tools),
            room=ctx.room,
